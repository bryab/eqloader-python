...
pub struct S3DMaterial {
    fragment: ArcRef<WldDoc, MaterialDef>,
}
...
impl S3DMaterial {
    ...
    fn iter_texture_filenames(&self) -> impl Iterator<Item = String> + '_ {
        let wld = self.get_wld();
        ...
        simplesprite
            ...
            .flat_map(|image| match image {
                ...
                Some(i) => i
                    ...
                    .map(|e| String::from(e.file_name.to_lowercase()))
                    .collect::<Vec<_>>(),
                None => vec![],
            })
    }
